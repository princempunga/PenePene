...
export default function CartIndex({ items, total }) {
    ...
    const cartError = checkoutError || errors?.cart;
                    ...
                    <div className="flex flex-col lg:flex-row gap-8">
                            ...
                            {items.map(item => (
                                <div key={item.id} className="bg-white rounded-2xl border border-gray-200 shadow-sm p-4 md:p-6 flex gap-4 items-start">
                                    ...
                                    <div className="hidden md:block text-right shrink-0">
                                        <p className="text-xs text-gray-500 mb-1">{t('cart.subtotal')}</p>
                        ...
                        <div className="w-full lg:w-80 shrink-0">
                            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 sticky top-28">
                                <h2 className="text-lg font-bold text-gray-900 mb-6">{t('cart.order_summary')}</h2>
                                ...
                                {!auth?.user && (
                                    <div className="mb-4 p-3 bg-amber-50 border border-amber-200 text-amber-800 rounded-xl text-sm">
                                        <strong>{t('cart.sign_in_required')}</strong>{t('cart.sign_in_to_checkout')}
                                    </div>
                                )}
                                ...
                                <form onSubmit={handleCheckout}>
                                    <button
                                        ...
                                        className="w-full bg-primary-600 hover:bg-primary-700 disabled:bg-primary-400 disabled:cursor-wait text-white font-bold py-4 rounded-xl transition-colors flex items-center justify-center gap-2 text-lg shadow-md"
                                    >
                                        {checkingOut ? (
                                            <>{t('cart.processing_order')}</>
                                        ) : auth?.user ? (
                                            <>{t('cart.confirm_order')} <ArrowRight size={20} /></>
                                        ) : (
                                            <>{t('cart.sign_in_to_checkout_btn')} <ArrowRight size={20} /></>
                                        )}
                                    </button>
                                </form>
