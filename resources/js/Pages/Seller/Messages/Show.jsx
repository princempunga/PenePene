...
export default function MessageShow({ conversation, messages }) {
    ...
    return (
        <>
            ...
            <SellerLayout>
                <div className="bg-white rounded-xl border border-gray-200 shadow-sm flex flex-col h-[75vh]">
                    {/* Header */}
                    <div className="flex items-center gap-3 p-4 border-b border-gray-100">
                        <Link href="/seller/messages" className="text-gray-500 hover:text-gray-700">
                            <ArrowLeft size={20} />
                        </Link>
                        <div className="w-10 h-10 bg-gray-100 rounded-full flex items-center justify-center font-bold text-gray-600 shrink-0">
                            {conversation.buyer?.user?.name?.charAt(0)?.toUpperCase()}
                        </div>
                        <div>
                            <p className="font-semibold text-gray-900">{conversation.buyer?.user?.name}</p>
