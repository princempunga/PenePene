...
export default function OrderShow({ order }) {
    ...
    const cancel = () => {
        if (confirm(t('orders.cancel_confirm'))) {
    ...
    const contactSeller = () => {
        ...
        router.post(`/buyer/orders/${order.id}/contact-seller`, {}, {
            onError: (pageErrors) => {
                const message = pageErrors?.contact
                    || Object.values(pageErrors ?? {})[0]
                    || t('orders.contact_error');
                alert(message);
            },
            onFinish: () => setContactingSeller(false),
        });
    };
    ...
    return (
        <AppLayout>
            <Head title={`${t('buyer.order_number')} ${order.order_number}`} />
            <div className="max-w-7xl mx-auto px-4 py-8">
                <div className="flex items-center gap-3 mb-6">
                    <Link href="/buyer/orders" className="text-sm text-gray-500 hover:text-primary-600">{t('orders.back_to_orders')}</Link>
                ...
                {errors?.contact && (
                    <div className="mb-6 p-3 bg-red-50 border border-red-200 text-red-800 rounded-lg text-sm">{errors.contact}</div>
                )}

                {order.status !== 'cancelled' && (
                    <div className="bg-white rounded-xl border border-gray-200 p-6 mb-6 shadow-sm">
                        <h2 className="font-bold text-gray-900 mb-5">{t('orders.order_status')}</h2>
                            ...
                            {statusSteps.map((step, index) => (
                                <React.Fragment key={step}>
                                    <div className="flex flex-col items-center flex-shrink-0">
                                        ...
                                        <p className={`text-xs mt-1.5 font-medium ${index <= currentStep ? 'text-primary-600' : 'text-gray-400'}`}>
                                            {statusLabel(step)}
                                        </p>
                                    </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div className="lg:col-span-2 space-y-4">
                        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                            <div className="p-5 border-b border-gray-100">
                                <h2 className="font-bold text-gray-900">{t('orders.items_ordered')}</h2>
                            </div>
                    ...
                    <div className="space-y-4">
                        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
                            <h2 className="font-bold text-gray-900 mb-4">{t('orders.order_summary')}</h2>
                            ...
                            <div className="mt-4 pt-4 border-t text-sm space-y-2">
                                <div className="flex justify-between text-gray-600">
                                    <span>{t('buyer.order_status')}</span>
                                    <span className="font-medium text-gray-900">{statusLabel(order.status)}</span>
                                </div>
                                <div className="flex justify-between text-gray-600">
                        ...
                        {order.seller && (
                            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
                                ...
                                <button
                                    ...
                                    className="mt-4 w-full text-sm font-medium bg-gray-100 hover:bg-gray-200 disabled:opacity-60 disabled:cursor-wait text-gray-800 py-2 rounded-lg transition-colors text-center"
                                >
                                    {contactingSeller ? t('orders.opening_chat') : t('orders.contact_seller')}
                                </button>
                            </div>
                        )}
                        ...
                        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5 space-y-3">
                            ...
                            {['pending', 'confirmed'].includes(order.status) && (
                                ...
                                >
                                    <X size={16} />
                                    {processing ? t('orders.cancelling') : t('orders.cancel_order')}
                                </button>
                            )}
                        </div>
                    </div>
